...
import RealisticGarmentPreview from '@/components/RealisticGarmentPreview';
import ProductDetailsView from '@/components/ProductDetailsView';
import ProductCategoryBar from '@/components/ProductCategoryBar';
import BagCategoryView from '@/components/BagCategoryView';
...
const RemnantAnalysis: React.FC = () => {
  ...
  const { fabricSource, setFabricSource } = useFabricStore();
  ...
  const handleTopCategorySelect = (catId: string) => {
    ...
    if (catId === 'All' || catId === 'Shirts') {
      setSelectedGarment('shirt');
    } else if (catId === 'T-Shirts') {
      setSelectedGarment('kids_shirt');
    } else if (catId === 'Pants') {
      setSelectedGarment('pants');
    } else if (catId === 'Shorts') {
      setSelectedGarment('shorts');
    } else if (catId === 'Dresses') {
      setSelectedGarment('dress');
    } else if (catId === 'Bags') {
      setSelectedGarment('bag');
  ...
  useEffect(() => {
    // Keep URL for 3D viewer across pages
  }, []);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) {
      setFabricSource({ type: 'color', image: null, imageFile: null });
      setFileName('');
      return;
    }
    ...
    if (fabricSource.image) {
      URL.revokeObjectURL(fabricSource.image);
    }

    const nextUrl = URL.createObjectURL(file);
    setFabricSource({ type: 'image', image: nextUrl, imageFile: file });
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const result = await analyzeFabric({ image: fabricSource.imageFile, width, length, shape, material: fabricSource.fabricType });
      ...
      <div className="min-h-screen bg-[#f3f5f3] px-6 py-6 text-slate-800">
        <ProductDetailsView 
          ...
          onSwitchProduct={(name) => {
            const match = analysis?.recommended_products.find((p) => p.pattern_name === name);
            if (match) {
              selectProduct(match);
            } else {
              selectProduct({
                pattern_id: name.toLowerCase().replace(/\s+/g, '-'),
                pattern_name: name,
                feasible: true,
                required_area_m2: 0.8,
                available_area_m2: Number(area),
                utilization_percentage: 82.5,
                used_area_m2: 0.75,
                category: 'Clothing',
                quantity_possible: 1,
                required_area: 0.8,
                remnant_area: Number(area),
                remaining_area: Number(area) - 0.75,
                confidence: 'High',
                placements: [],
                waste_percentage: 17.5,
                ai_recommended: true,
                pattern_pieces: [],
                ai_reason: 'Suitable based on estimated sizing.',
                infeasibility_reason: undefined
              });
            }
          }}
        />
  ...
  return (
    <div className="min-h-screen bg-[#f3f5f3] px-6 py-6 text-slate-800">
      <div className="mx-auto max-w-[1600px]">
        ...
        <p className="mb-6 text-[1.05rem] text-slate-600">Find out what your leftover fabric can become.</p>

        {/* Top Product Category Filter Bar */}
        ...
        <div className="grid gap-8 lg:grid-cols-[2fr_3fr]">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <select
                    value={fabricSource.fabricType}
                    onChange={(e) => {
                      setFabricSource({ fabricType: e.target.value });
                      if (!fabricSource.image) setFabricSource({ type: 'color' });
                    }}
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-base text-slate-800 outline-none"
                  >
                  ...
                  <input
                    type="color"
                    value={fabricSource.color}
                    onChange={(e) => {
                      setFabricSource({ color: e.target.value });
                      if (!fabricSource.image) setFabricSource({ type: 'color' });
                    }}
                    className="h-12 w-12 cursor-pointer rounded-xl border border-slate-200 bg-slate-50 p-1"
                  />
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {!isAnalyzed ? (
              <>
                {fabricSource.image || fabricSource.type === 'color' ? (
                  <div className="grid gap-6 lg:grid-cols-2">
                    <RealisticGarmentPreview garmentType={selectedGarment} />
                    <Garment3DViewer
                      // @ts-ignore
                      garmentType={selectedGarment} fabricUrl={fabricSource.image} fabricColor={fabricSource.color} />
                  </div>
                ) : (
                  <div className="flex min-h-[250px] items-center justify-center">
                    <div className="relative flex h-52 w-52 items-center justify-center border-[3px] border-[#7dc69a] bg-[#e8f7ee] bg-[linear-gradient(90deg,rgba(125,198,154,0.15)_1px,transparent_1px),linear-gradient(rgba(125,198,154,0.15)_1px,transparent_1px)] bg-[size:18px_18px]">
                      <div className="text-center text-slate-800">
                        ...
                        <div className="mt-2 text-sm text-slate-500">{fabricSource.fabricType}</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-6 grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-xl bg-slate-100 p-3 text-center">
                    ...
                    <div className="mt-1 text-base font-semibold text-slate-800">{fabricSource.fabricType}</div>
                  </div>
                </div>
              </>
            ) : (
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {(analysis?.recommended_products || []).slice(0, 6).map((product) => (
                      <button key={product.pattern_id} type="button" onClick={() => { selectProduct(product); setShowDetails(true); }} className={`rounded-xl border p-4 text-left ${selectedProduct?.pattern_id === product.pattern_id ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                ...
                {selectedProduct && (
                  <>
                    ...
                    <CuttingLayout product={selectedProduct} width={width} length={length} fabricSource={fabricSource} />
                  </>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 text-sm text-slate-700">
                  <div className="rounded-xl bg-slate-100 p-3">
                    ...
                    <div className="mt-1 font-semibold text-slate-800">{fabricSource.fabricType}</div>
                  </div>
                  <div className="rounded-xl bg-slate-100 p-3">
...
const CuttingLayout = ({ product, width, length, fabricSource }: { product: FabricProductMatch; width: number; length: number; fabricSource?: any }) => {
  const scale = Math.min(360 / width, 280 / length);
  const backgroundStyle = fabricSource?.image 
    ? { backgroundImage: `url(${fabricSource.image})`, backgroundSize: 'cover' }
    : fabricSource?.color
      ? { backgroundColor: fabricSource.color }
      : {};
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-xl bg-slate-100 p-4">
        <div className="relative mx-auto border-2 border-emerald-500 bg-emerald-50" style={{ width: width * scale, height: length * scale, ...backgroundStyle }}>
